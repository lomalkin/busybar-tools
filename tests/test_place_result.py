"""Unit tests for fetch destination placement."""
import os

from busybar_tools.firmware import _place_result


def test_no_output_returns_src_unchanged(tmp_path):
    f = tmp_path / "a.tgz"
    f.write_text("x")
    assert _place_result(str(f), None) == str(f)
...
def test_file_to_trailing_slash_dir(tmp_path):
    ...
    f.write_text("x")
    out = tmp_path / "dst"
    res = _place_result(str(f), str(out) + os.sep)
...
def test_file_to_existing_dir(tmp_path):
    ...
    out = tmp_path / "dst"
    out.mkdir()
    res = _place_result(str(f), str(out))
    assert res == str(out / "a.tgz")
    assert os.path.isfile(res)
...
def test_file_to_explicit_filename_creates_parents(tmp_path):
    ...
    res = _place_result(str(f), str(dst))
    assert res == str(dst)
...
def test_dir_copied_to_output(tmp_path):
    ...
    out = tmp_path / "out"
    res = _place_result(str(src), str(out))
    assert res == str(out)
