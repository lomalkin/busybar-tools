...
from busybar_tools.commands.cli_terminal import _run_cli_batch
from busybar_tools.device import DETECT_FIELDS, VERSION_FIELDS


def test_device_read_info_returns_complete(device_endpoint):
    info = bt.read_device_info(
        device_endpoint, retries=5, delay=1,
...
def test_cli_args_form_runs_command(device_endpoint, capsys):
    ret = bt.run_cli_terminal(bt.CliOptions(device_endpoint, wait_before=False, commands=("--", "device_info")))
    out = capsys.readouterr().out
    assert ret in (0, None)
    assert "u5_firmware_target" in out


def test_cli_stdin_form_runs_commands(device_endpoint, monkeypatch, capsys):
    stdin = io.StringIO("device_info\nuptime\n")
    monkeypatch.setattr("sys.stdin", stdin)
    bt.run_cli_terminal(bt.CliOptions(device_endpoint, wait_before=False))
    out = capsys.readouterr().out
    assert "u5_firmware_target" in out
    assert "Uptime" in out


def test_run_cli_batch_reads_response(device_endpoint, capsys):
    ret = _run_cli_batch(bt.CliOptions(device_endpoint, wait_before=False), ["device_info"])
    out = capsys.readouterr().out
    assert ret == 0
    assert "u5_firmware_target" in out
