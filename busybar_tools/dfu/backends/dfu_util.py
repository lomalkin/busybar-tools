...
# Restrict dfu-util to the STM32 bootloader IDs; with several DFU devices connected
# dfu-util itself refuses to pick one at random and asks for a serial number.
_DFU_DEVICE_FILTER = f"{DFU_VENDOR_ID:04x}:{DFU_PRODUCT_ID:04x}"
...
class DfuUtilBackend:
    ...
    def find_devices(self):
        ...
        return "found dfu" in text or _DFU_DEVICE_FILTER in text
    ...
    def program_firmware(self, fw_file, reset=False):
        ...
        cmd = [self.executable, "-d", _DFU_DEVICE_FILTER, "-a", "0", "-D", fw_file]
        if reset:
            cmd.append("-R")
    ...
    def leave_dfu(self, address=DFU_LEAVE_ADDRESS):
        ...
        try:
            ...
            cmd = [self.executable, "-d", _DFU_DEVICE_FILTER, "-a", "0", "-s", f"0x{address:08x}:leave", "-D", tmp_path]
...
def ensure_dfu_util(executable=None, auto_install=False):
    """Return a working DfuUtilBackend; installing dfu-util is strictly opt-in."""
    if executable:
